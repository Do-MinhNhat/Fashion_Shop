        ...
        <table class="w-full text-left border-collapse">
            <thead class="bg-gray-100 border-b border-gray-200">
                <tr>
                    ...
                    <th class="p-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Tên liên hệ</th>
                    <th class="p-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">URL</th>
            ...
            <tbody class="divide-y divide-gray-100" x-data="{}">
                ...
                <tr class="group hover:bg-blue-50 transition border-b transition-colors duration-300">
                    ...
                    <td class="p-4">
                        <p class="font-medium text-gray-900 group-hover:text-blue-600 transition">{{ $contact->name }}</p>
                    </td>
                    <td class="p-4">
                        <p class="font-medium text-gray-900 group-hover:text-blue-600 transition">{{ $contact->url }}</p>
                    </td>
                    ...
                    <td class="p-4 flex justify-end gap-2">
                        <form action="{{ route('admin.contact.restore', $contact) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('PUT')
                        ...
                        <form action="{{ route('admin.contact.forceDelete', $contact) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
