<div x-data="{ open: {{ $errors->edit->any()? 'true' : 'false' }}, oldData: {{ $errors->edit->any()? json_encode(old()) : json_encode((object)[]) }}, tag: {} }"
    x-show="open"
    x-on:open-edit-modal.window="open = true; tag = $event.detail"
    style="display: none;"
                ...
                <div class="overflow-y-auto max-h-[79vh] flex-grow">
                    <div class="flex-col overflow-y-auto max-w-2xl">
                        <div class="flex justify-between items-center p-6 border-b">
                            <h3 class="text-lg font-bold mr-2">Sửa nhãn</h3>
                        ...
                        <div class="p-6 overflow-y-auto custom-scrollbar">
                            <form method="POST" :action="'{{ route('admin.tag.update', ['tag' => ':slug']) }}'.replace(':slug', oldData.slug)" x-ref="tagForm" class="space-y-6" @submit="handleSubmit($event)">
                                @method('PUT')
                                @csrf
                                ...
                                <div class="border rounded-xl grid grid-cols-1 md:grid-cols-1 p-5">
                                    <div class=" grid grid-cols-1 md:grid-cols-1">
                                        <div>
                                            <label class="text-xs font-semibold uppercase text-gray-500">Tên nhãn</label>
                                            <span x-ref="nameError" class="text-xs text-red-500 italic">*</span>
                                            <input x-model="oldData.name" x-ref="nameInput" type="text" name="name" class="w-full p-2.5 border rounded text-sm" placeholder="Nhập tên nhãn" maxlength="255">
                                        </div>
                                    </div>
                                </div>
...
@push('scripts')
<script>
    function tagEditManager() {
        return {
            init() {
                this.$watch('tag', (tag) => {
                    this.oldData.name = String(tag.name);
                    this.oldData.slug = String(tag.slug);
                    this.oldData.status = String(tag.status);
                })
            },
            handleSubmit(e) {
                ...
                if (!name.value) {
                    ...
                    nameMsg.innerHTML = "Tên nhãn không được để trống!";
