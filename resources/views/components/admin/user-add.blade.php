@props(['roles'])
<div x-data="{ open: {{ $errors->add->any()? 'true' : 'false' }}, oldData: {{ $errors->add->any()? json_encode(old()) : json_encode((object)['status' => 1, 'review' => 1, 'gender' => 1]) }} }"
    x-show="open"
                ...
                <div class="overflow-y-auto max-h-[79vh] flex-grow">
                    <div class="flex-col overflow-y-auto max-w-2xl">
                        ...
                        <div class="p-6 overflow-y-auto custom-scrollbar">
                            <form method="POST" action="{{ route('admin.user.store') }}" x-ref="userForm" class="space-y-6" @submit="handleSubmit($event)">
                                @csrf
                                ...
                                <div class=" grid grid-cols-1 md:grid-cols-1">
                                    <div>
                                        ...
                                        <input x-model="oldData.email" x-ref="emailInput" type="email" name="email" class="w-full p-2.5 border rounded text-sm" placeholder="Nhập email (tên đăng nhập)" maxlength="255" required>
                                    </div>
                                </div>
                                <div class=" grid grid-cols-1 md:grid-cols-1">
                                    <div>
                                        ...
                                        <input x-model="oldData.phone" x-ref="phoneInput" type="text" name="phone" @input="oldData.phone = $event.target.value.replace(/[^0-9]/g, '')" class="w-full p-2.5 border rounded text-sm" placeholder="Nhập số điện thoại" maxlength="10" minlength="10" required>
                                    </div>
                                </div>
                                <div class=" grid grid-cols-1 md:grid-cols-1">
                                    <div>
                                        <label class="text-xs font-semibold uppercase text-gray-500">Mật khẩu</label>
                                        <span x-ref="passwordError" class="text-xs text-red-500 italic">*</span>
                                        <input x-ref="passwordInput" type="password" name="password" class="w-full p-2.5 border rounded text-sm" placeholder="Nhập mật khẩu" maxlength="255">
                                    </div>
                                </div>
                                <div class=" grid grid-cols-1 md:grid-cols-1">
                                    <div>
                                        <label class="text-xs font-semibold uppercase text-gray-500">Nhập lại mật khẩu</label>
                                        <span x-ref="passwordComfirmError" class="text-xs text-red-500 italic">*</span>
                                        <input x-ref="passwordConfirmationInput" type="password" name="password_confirmation" class="w-full p-2.5 border rounded text-sm" placeholder="Nhập lại mật khẩu" maxlength="255">
                                    </div>
                                </div>
    ...
    function roleAddManager() {
        return {
            ...
            handleSubmit(e) {
                ...
                const passwordComfirm = this.$refs.passwordConfirmationInput;
                ...
                if (!password.value) {
                    e.preventDefault();
                    const passwordMsg = this.$refs.passwordError;
                    passwordMsg.innerHTML = "Mật khẩu được để trống!";
                    passwordMsg.scrollIntoView({
                        behavior: 'smooth',
                        block: 'center'
                    })
                }
                if (!passwordComfirm.value) {
                    e.preventDefault();
                    const passwordComfirmMsg = this.$refs.passwordComfirmationError;
                    passwordComfirmMsg.innerHTML = "Yêu cầu nhập lại mất khẩu!";
                    passwordComfirmMsg.scrollIntoView({
                        behavior: 'smooth',
                        block: 'center'
                    })
                }
